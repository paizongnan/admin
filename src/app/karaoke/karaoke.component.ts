...
export class karaokeComponent implements OnInit {  
    ...
    ngOnInit(): void {
       ...
       var obj = {
        ...
        this._appservice.getKaraokeList(obj, this.admintoken).subscribe((Response) => {
             ...
             if (resp.response_code===4002) {    // block for jwt session timeout
    ...
    loadmore() {    
       ...
       // console.log(obj)
        ...
        this._appservice.getKaraokeList(obj, this.admintoken).subscribe((Response) => {
            ...
            var result = (Response as any).response_data
            // console.log(result);
            for (var index = 0; index < result.length; index++) {
                var dataarrval:{[key:string]: any} = {}
                dataarrval['_id'] = result[index]._id
    ...
    onImageFileSelected (fileInput: any) {
     // console.log(fileInput);       
        let myFile = fileInput.target.files[0];
        this.albumimage=myFile;  
    }
    onMusicFileSelected (fileInput: any) {
     // console.log(fileInput);
        let myFile = fileInput.target.files[0];
        this.karaokefile=myFile;       
    }
    onlyricFileSelected (fileInput: any) {
     // console.log(fileInput);        
        let myFile = fileInput.target.files[0];
        this.lyricfile=myFile;      
    }
    onDanceFileSelected (fileInput: any) {
        // console.log(fileInput);
        let myFile = fileInput.target.files[0];
